...
{
  var url = "http://www.students.oamk.fi/~t7trle00/demo-master/index.php/api/host/listinguniqu";
  ...
  xhttp.onreadystatechange=function()
  {
      if(this.readyState == 4 && this.status == 200)
      {
          ...
          data='<div class="row text-center animate-fadein">' ;
          for( x in jsonData)
          {
...
{
    this.carID = carID ;
    var url = "http://www.students.oamk.fi/~t7trle00/demo-master/index.php/api/host/listinguniqu/" +"carid/" + this.carID ;
    var xhttp = new XMLHttpRequest() ;
    xhttp.open("GET",url,true) ;
    ...
    var data = "" ;
    var cover_url = "http://www.students.oamk.fi/~t7trle00/demo-master/cover_gallery/" ;
    var other_url = "http://www.students.oamk.fi/~t7trle00/demo-master/other_gallery/" ;
    xhttp.onreadystatechange = function()
    {
        if(xhttp.readyState == 4 && xhttp.status == 200)
        {
            ...
            jsonData= JSON.parse(xhttp.responseText) ;
                data +=
                        ...
                        '<div class="show_detail animate-fadein">' +
                        '<div class="row">' +
                        ...
                        '</div>' +
                        '<div class="row"><p>Starting date available</p>'+
				'<div class="input-group date col-sm-6">' +
                            ...
                            '</span>' +
                        '</div></div>' +
                        '<div class="row"><p>Last date available</p>'+
				'<div class="input-group date col-sm-6">' +
...
//host show all by userid
function getListing(format)
{
    var userid = document.getElementById("userID").value ;
    var url = "http://www.students.oamk.fi/~t7trle00/demo-master/index.php/api/host/listinguniqu/userid/" + userid;
 	  var xhttp = new XMLHttpRequest();
    xhttp.open("GET", url,true);
    var jsonData = "" ;
    var cover_url = "http://www.students.oamk.fi/~t7trle00/demo-master/cover_gallery/" ;
    var data = '<table border="1" align="center" class="getListing animate-fadein">'+
                '<tr>'+
                    '<th>CarID</th>' +
                    '<th>TITLE</th>' +
                    '<th>COVER PHOTO</th>' +
    ...
    xhttp.onreadystatechange=function()
    {
        if(this.readyState == 4 && this.status == 200)
        {
            ...
            {
              data_empty = '<p style="font-size:1.25em;">You do not have any listing to show. Go to '+"<b>Add New Listing</b>" +
                           " to create your first listing</p>" ;
              document.getElementById("results").innerHTML = data_empty ;
            }
            else
            {
              for( x in jsonData)
              ...
              '<td><button class="btn btn-info" onclick="ShowListingHost(' +
                      jsonData[x].carID +','+ userid + ')">SHOW</button></td>' +
              '</tr>' ;
              }
              data += '</table>' ;
              document.getElementById("results").innerHTML = data ;
...
function ShowListingHost(carID,userid)
{
    this.carID = carID ;
    var url = "http://www.students.oamk.fi/~t7trle00/demo-master/index.php/api/host/listinguniqu/userid/" +userid + "/carid/"+ this.carID ;
    var xhttp = new XMLHttpRequest() ;
    xhttp.open("GET",url,true) ;
...
{
    this.del_id = carID ;
    var url = "http://www.students.oamk.fi/~t7trle00/demo-master/index.php/api/host/listinguniqu/carid/" + this.del_id ;
	var xhttp = new XMLHttpRequest();
...
function AddListing()
{
    var url = "http://www.students.oamk.fi/~t7trle00/demo-master/index.php/api/host/listing";
	  var xhttp = new XMLHttpRequest();
