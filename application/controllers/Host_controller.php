...
class Host_controller extends CI_Controller {
 ...
 public function __construct()
 {
   ...
   $this->load->helper('form') ;
	...
	{
    $data['userID'] = (int)$this->session->id ;
  ...
  public function get_listing_update($userID)
  {
    $data['userID'] = (int)$this->session->id ;
    $data['images']=$this->Host_model->get_userlisting($userID);
  ...
  public function listing_detail($carID)
  {
    $data['carID'] =$carID ;
    $data['userID'] = (int)$this->session->id ;
    $data['id_get_edit'] = $this->Host_model->get_car($carID) ;
